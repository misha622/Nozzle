...
from nozzle.scheduler.tasks.training import retrain_model
from nozzle.scheduler.tasks.reports import generate_weekly_report
...
async def ingest_all_sources(ctx):
    ...
    async with async_session_factory() as db:
        ...
        result = await db.execute(select(Source).where(Source.status == SourceStatus.ACTIVE))
        sources = result.scalars().all()
        for source in sources:
            try:
                service = IngestionService(db)
                stats = await service.ingest_from_source(source_id=str(source.id), limit=1000)
...
class WorkerSettings:
    functions = [ingest_all_sources, run_clustering, retrain_model, generate_weekly_report]
    cron_jobs = [
        cron(ingest_all_sources, minute="*/5", run_at_startup=False),
        cron(run_clustering, minute="*/15", run_at_startup=False),
        cron(retrain_model, hour="*/6", run_at_startup=False),
        cron(generate_weekly_report, weekday=0, hour=9, run_at_startup=False),
    ]
    on_startup = startup
